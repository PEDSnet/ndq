...
#'   \item{table_prev}{**ONLY REQUIRED IF PREV_CT_SRC = CDM**; The name of the CDM or pre-computed results table to be used to retrieve the counts associated with the previous data model}
#'   \item{filter_logic}{**OPTIONAL** The logic that should be applied to BOTH table_current and table_prev in order to tailor the tables to the desired check assessment}
#'   \item{check_domain}{A string indicating the domain of the check (ex: medadmin_drugs, prescription_drugs)}
...
#' \describe{
#'   \item{check_id}{A short string "code" used to identify the specific check (ex: de, de_rx)}
#'   \item{check_description}{A longer description of the check}
...
#'   \item{table}{The name of the CDM or pre-computed results table where the relevant data is kept}
#'   \item{acceptable_vocabularies}{A comma-separated string of acceptable vocabulary values (ex: "RxNorm, RxNorm Extension")}
#'   \item{concept_field}{The *_concept_id field with the concepts that should be evaluated (ex: rxnorm_cui, raw_rxnorm_cui)}
#'   \item{filter_logic}{**OPTIONAL** The logic that should be applied to the provided table in order to tailor the tables to the desired check assessment (ex: if certain drugs had different vocabulary requirements than others)}
#' }
#'
"vc_input_pcornet"
...
#'   \item{source_value_field}{For analyses where `produce_mapped_list == TRUE`, the *_source_value field linked to the concept_field of interest to be used to produce a list of source values associated with unmapped concepts}
#'   \item{date_field}{For analyses where `by_year == TRUE`, the date field to be used to retrieve the year associted with the unmapped concepts}
#'   \item{filter_logic}{**OPTIONAL** The logic that should be applied to the provided table in order to tailor the tables to the desired check assessment (ex: if you only want to identify billed procedures that are unmapped)}
#' }
#'
"uc_input_pcornet"
...
#'   \item{schema}{The schema where the data is kept. Use `cdm` to use the pre-configured `cdm_schema`, `result` to use the preconfigured `results_schema`, or input the exact name of the schema.}
#'   \item{table}{The name of the CDM or pre-computed results table where the relevant data is kept}
#'   \item{filter_logic}{**OPTIONAL** The logic that should be applied to the provided table in order to tailor the tables to the desired check assessment (ex: if you only want to assess billed procedures)}
#' }
#'
"cfd_input_pcornet"
...
#' with a single visit type. The name of the second column will control
#' where the visit filtering takes place, so you can filter on other
#' columns in the visit_tbl if need be
#'
#' @format ## `cfd_visit_types_pcornet`
#' A dataframe or CSV file with 2 columns
#' \describe{
#'   \item{visit_type}{A string to identify the visit type of interest (ex: inpatient)}
#'   \item{enc_type}{The enc_type associated with the visit type of interest (ex: 9202)}
#' }
#'
...
#'   \item{table}{The name of the CDM or pre-computed results table where the relevant data is kept}
#'   \item{concept_field}{The *_concept_id field with the concepts that make up the valueset (ex: drug_concept_id, drug_source_concept_id)}
#'   \item{concept_table_field}{**OPTIONAL** The field in OMOP's vocabulary concept table that should be used to identify the concept_ids in the concept_field (i.e. concept_name, concept_class_id); only needed if concept_tbl is not NULL}
#'   \item{filter_logic}{**OPTIONAL** The logic that should be applied to the provided table in order to tailor the tables to the desired check assessment (ex: if you only want to assess prescription drugs)}
#' }
#'
"bmc_input_omop"
...
#'   \item{schema}{The schema where the data is kept. Use `cdm` to use the pre-configured `cdm_schema`, `result` to use the preconfigured `results_schema`, or input the exact name of the schema.}
#'   \item{table}{The name of the CDM or pre-computed results table where the relevant data is kept}
#'   \item{concept_field}{The field with the concept codes that make up the valueset (ex: rxnorm_cui, dx)}
#'   \item{concept_table_field}{**OPTIONAL** The field in OHDSI's vocabulary concept table that should be used to identify the concept_ids in the concept_field (i.e. concept_name, concept_class_id); only needed if concept_tbl is not NULL}
#'   \item{filter_logic}{**OPTIONAL** The logic that should be applied to the provided table in order to tailor the tables to the desired check assessment (ex: if you only want to assess billed procedures)}
#' }
#'
"bmc_input_pcornet"
...
#' concepts in the result of this module. This exact
#' file is also included as a CSV in the package if the user wishes
#' to use it, or the structure can be copied to produce a custom
#' list of checks.
#'
#' @format ## `bmc_best_notbest`
#' A dataframe or CSV file with 4 columns
#' \describe{
#'   \item{check_name}{The name of the check associated with the concepts of interest, which should match what is provided in the `bmc_tbl` input}
#'   \item{concept}{The concept identifier or concept name (depending on if a vocabulary table is provided) that should be classified as best/not best}
#'   \item{best_notbest}{A binary indicator where 0 indicates the concept is not best / unacceptable and 1 indicates the concept is best / acceptable}
#'   \item{default_to}{An indicator, either `best` or `notbest`, to designate what any concepts not listed in this file should be defaulted to}
#' }
#'
"bmc_best_notbest"
...
#'
#' A sample version of an OMOP compliant input file for
#' the Date Plausibility data quality module. This exact
#' file is also included as a CSV in the package if the user wishes
#' to use it, or the structure can be copied to produce a custom
#' list of checks.
#'
#' @format ## `dp_input_omop`
#' A dataframe or CSV file with 6 columns
#' \describe{
#'   \item{check_id}{A short string "code" used to identify the specific check (ex: de, de_rx)}
#'   \item{check_description}{A longer description of the check (ex: full drug_exposure table, prescribed drugs based on drug_type_concept_id)}
